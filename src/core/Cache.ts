...
export const Cache = {
  ...
  purgeInterval: null as unknown as ReturnType<typeof setInterval>,

  init() {
    if (ThreeCache.enabled) {
      console.warn("ThreeJS cache should be disabled");
      ThreeCache.enabled = false;
    }
  ...
  get(url: string, key: string): HTMLImageElement | Blob | undefined {
  ...
  purge() {
    Object.entries(this.items)
      ...
      .forEach(([key, { lastAccess }], index) => {
        ...
        if (
          index > 0 &&
          (Date.now() - lastAccess >= this.ttl * 1000 || index >= this.maxItems)
        ) {
          delete this.items[key];
        }
      });
